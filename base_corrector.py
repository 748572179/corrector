...
import re
import time
from keyword_corrector import correct_al
...
char_sim = CharFuncs(CorrectorConfig.char_meta_file)
regulars = compile_regulars()

def correct_all(texts, mask):
    results = correct_al(texts=texts,err_positions=mask)
    return results

def preprocess(texts, probs):
    text_to_process = []
    text_to_pass = []
    err_positions = []

    for i, (sent, p) in enumerate(zip(texts, probs)):
        if not do_correct_filter(sent):
            text_to_pass.append((i, sent))
        else:
            err_pos = find_err_pos_by_prob(p)
            if not err_pos:
                text_to_pass.append((i, sent))
            else:
...
def do_correct_filter(text):
    if len(re.findall(regulars['chinese'],text)) < 2:
        return False
    return True


def find_err_pos_by_prob(prob):
    if not prob:
        return []
    err_pos = [i for i, p in enumerate(prob) if p < 0.7]

    return err_pos

def correct(texts, probs):
        # 前处理，过滤无需纠错的文本
    if probs:
        texts_to_correct, err_positions, texts_to_pass = preprocess(texts, probs)
    else:
        texts_to_correct, err_positions, texts_to_pass = preprocess_non_prob(texts)
    if not texts_to_correct:
        return texts
        # 纠错
    after_correct = correct_all(texts_to_correct, err_positions)
        # 后处理，恢复原文本顺序
    after_correct = post_process(after_correct, texts_to_pass)
